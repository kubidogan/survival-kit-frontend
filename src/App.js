...
// This version shows 1 marker the pop up one but errors in console
...
// This Version shows 2 markers, one pop up and one normal
import React, { useRef, useEffect, useState } from "react";
import mapboxgl from "!mapbox-gl"; // eslint-disable-line import/no-webpack-loader-syntax
import Kits from "./components/Kits";
import Header from "./components/Header";
...
export default function App() {
  ...
  const addMarkersToMap = (survivalKits) => {
    if (map.current) {
      ...
      if (!map.current.hasImage("marker-15")) {
        map.current.loadImage(
          ...
          (error, image) => {
            ...
            if (map.current) {
              // Add the image only if it doesn't exist
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        map.current.on("load", () => {
          // Check if the source and layer already exist, remove them if they do
          if (map.current.getLayer("markers")) {
            map.current.removeLayer("markers");
          }
